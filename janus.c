...
static int janus_process_srtc_error_string(janus_request *request, char *response_type, uint64_t session_id, const char *transaction, gint error, gchar *error_string);
...
static int janus_process_srtc_error_string_tmp(janus_request *request, uint64_t session_id, const char *transaction, gint error, gchar *error_string)
{
	if(!request)
		return -1;
	/* Done preparing error */
...
static int janus_process_srtc_error_string(janus_request *request, char* response_type, uint64_t session_id, const char *transaction, gint error, gchar *error_string)
{
	...
	JANUS_LOG(LOG_VERB, "[%s] Returning %s API error %d (%s)\n", transaction, request->admin ? "admin" : "Janus", error, error_string);
	/* Prepare JSON error */
	json_t *reply = janus_create_srtc_response_message("response",response_type, error, session_id, transaction);
	json_object_set_new(reply, "errormsg", json_string(error_string));
	/* Pass to the right transport plugin */
	return request->transport->send_message(request->instance, request->request_id, request->admin, reply);
}


int janus_process_srtc_error(janus_request *request, char* response_type , uint64_t session_id, const char *transaction, gint error, const char *format, ...)
{
	if(!request)
		return -1;
	...
	return janus_process_srtc_error_string(request, response_type, session_id, transaction, error, error_string);
